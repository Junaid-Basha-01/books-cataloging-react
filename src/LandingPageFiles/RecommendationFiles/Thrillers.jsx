...
import thrillerWorks from './thrillers.json'
import { Link } from 'react-router-dom'


function Thrillers() {

    const [thrillers, setThrillers] = useState(thrillerWorks.filter((__, idx)=> idx < 6))
    const [carouselNo, SetCarouselNo] = useState(0)
    const [books, setBooks] = useState([])

    useEffect(()=>{
        setBooks(thrillers.map((ele)=>{
            return (
    ...
    function leftClick(){
        if(carouselNo === 1){
            SetCarouselNo(0)
            setThrillers(thrillerWorks.filter((__, idx)=> idx < 6))
        }
    ...
    function rightClick(){
        if(carouselNo === 0){
            SetCarouselNo(1)
            setThrillers(thrillerWorks.filter((__, idx)=>{return idx >= 6 && idx < 12}))
        } 
    }
    return (
        <>
            ...
            <div className='flex justify-between bg-cyan-100 my-3' id='container-thrillers'>
